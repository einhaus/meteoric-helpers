...
function containsNodeImports(filePath: string): boolean {
  try {
    ...
    for (const nodeModule of nodeModules) {
      ...
      if (namespaceImportRegex.test(content) || importRegex.test(content) || 
          sideEffectRegex.test(content) || requireRegex.test(content) || 
...
console.log(`- Created node barrel file with ${allExports.length + nodeSpecificExports.length} total exports (${nodeSpecificExports.length} Node.js-specific)`);
